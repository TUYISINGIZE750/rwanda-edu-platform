"""Setup schools database with proper structure"""
import sys
import os
...
def setup_database():
    """Create tables and load TVET schools"""
    print("Creating database tables...")
    Base.metadata.create_all(bind=engine)
    print("Tables created successfully")
    
    # Load TVET schools from Excel
    ...
    try:
        ...
        print(f"\nTesting district filtering:")
        districts = db.query(School.district).distinct().limit(5).all()
        for (district,) in districts:
            count = db.query(School).filter(School.district == district).count()
